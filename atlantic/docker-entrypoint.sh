...
if [ -f "$PHAROS_CONF" ]; then
    # Try to extract meta_data from pharos.conf
    # This assumes the config has: "meta_data": "../data" or similar
    ...
    if [ -n "$CONFIG_DATA_PATH" ]; then
        ...
        echo "Warning: Could not extract meta_data from pharos.conf"
        BOOTSTRAPPED=false
    fi
else
