...
"""
    ...
    Date: 2022/12/06
"""
from aldaba_ops.toolkit import compose, template, config, schema, svc, utils, const, aldaba, logs
...
class ConfigManager(object):
    ...
    __service_enum = {
        const.SERVICE_PORTAL: aldaba.ServiceEnum.PORTAL,
        const.SERVICE_CONTROLLER: aldaba.ServiceEnum.CONTROLLER,
        const.SERVICE_TXPOOL: aldaba.ServiceEnum.TXPOOL,
        const.SERVICE_COMPUTE: aldaba.ServiceEnum.COMPUTE,
        const.SERVICE_LIGHT: aldaba.ServiceEnum.LIGHT,
    }
    def gen_service_id(self, service: str, idx: int) -> str:
        ...
        if service not in self.__service_enum:
            return service_name
        uuid = aldaba.get_pod_uuid(self.__service_enum[service], idx)
