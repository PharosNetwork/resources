...
from aldaba_ops.toolkit import command, svc, const, logs, utils
from aldaba_ops.toolkit.schemas.domain import *
from aldaba_ops.toolkit.conn_group import is_local, ConcurrentGroup, local
from aldaba_ops.toolkit.utils import safe_get_nested
from aldaba_ops.toolkit.conf import Generator
...
class Composer(object):
    ...
    def __init__(self, domain_file: str):
        ...
        for host, inst_list in self._all_instances.items():
            ...
            for inst in inst_list:
                image_name = 'aldaba_light' if self._is_light else 'aldaba'
                docker_image = f'{self._domain.docker.registry}/{image_name}:{self._domain.version}'
                command = f'./aldaba -s {inst.service}'
                vols = ['conf', 'data', 'log']
                ...
                dc_services[inst.name] = {
                    ...
                    'volumes': [{'type': 'bind', 'source': join(inst.dir, v), 'target': f'/aldaba/{v}'} for v in vols],
                    'restart': 'always'
                }
                if inst.service == const.SERVICE_ETCD:
                    dc_services[inst.name]['environment']['ETCD_DATA_DIR'] = '/aldaba/data/etcd'
                    dc_services[inst.name]['environment']['ETCD_LOG_OUTPUTS'] = '/aldaba/log/etcd.log'
                if command:
        ...
        if self.enable_docker:
            ...
            for binary in binaries:
                if backup:
                    ...
                    conn.run(f'mv {deploy_bin_dir}/{const.ALDABA_VERSION} {deploy_bin_dir}/{const.ALDABA_VERSION}_bak')
                conn.sync(self._build_file('bin', binary), deploy_bin_dir)
                conn.sync(self._build_file('bin', const.ALDABA_VERSION), deploy_bin_dir)
                if self.chain_protocol == const.PROTOCOL_EVM or self.chain_protocol == const.PROTOCOL_ALL:
                    if backup:
                        ...
                        conn.run(f'mv {deploy_bin_dir}/{const.DORA_SO} {deploy_bin_dir}/{const.DORA_SO}_bak')
                    conn.sync(self._build_file('bin', const.EVMONE_SO), deploy_bin_dir)
                    conn.sync(self._build_file('bin', const.DORA_SO), deploy_bin_dir)
            ...
            for instance in instances:
                ...
                # link DORA SO
                if self.chain_protocol == const.PROTOCOL_EVM or self.chain_protocol == const.PROTOCOL_ALL:
                    source = join(self.deploy_dir, 'bin', const.DORA_SO)
                    target = join(instance.dir, 'bin')
                    cmd = f'ln -sf {source} {target}'
                    logs.info(cmd)
                    conn.run(cmd)
                ...
                # link VERSION
                source = join(self.deploy_dir, 'bin', const.ALDABA_VERSION)
                target = join(instance.dir, 'bin')
                cmd = f'ln -sf {source} {target}'
                logs.info(cmd)
                conn.run(cmd)
    ...
    def deploy_host(self, conn: Connection, service=None, deploy_binary=True, deploy_conf=True):
        ...
        # make aldaba root workspace
        self._make_workspace(self.deploy_dir, 'bin', 'conf',
                             conn=conn)  # aldaba root workspace
    ...
    def deploy_local_cli(self):
        logs.info(f'deploy aldaba client at localhost:{self.local_client_dir}')
        ...
        local.sync(self._build_file(
            'bin', const.ALDABA_CLI), common_cli_bin_dir)
        local.sync(self._build_file(
            'bin', const.EVMONE_SO), common_cli_bin_dir)
        local.sync(self._build_file(
            'bin', const.DORA_SO), common_cli_bin_dir)
        local.sync(self._build_file(
            'bin', const.ALDABA_VERSION), common_cli_bin_dir)
        local.sync(self._build_file(
        ...
        local.run(f'cd {cli_bin_dir};ln -sf ../../../bin/{const.ALDABA_CLI}')
        local.run(f'cd {cli_bin_dir};ln -sf ../../../bin/{const.EVMONE_SO}')
        local.run(f'cd {cli_bin_dir};ln -sf ../../../bin/{const.DORA_SO}')
        local.run(f'cd {cli_bin_dir};ln -sf ../../../bin/{const.ALDABA_VERSION}')
        local.run(f'cd {cli_bin_dir};ln -sf ../../../bin/{const.ETCD_CTL_BIN}')
        local.run(f'cd {cli_bin_dir};ln -sf ../../../bin/{const.SVC_META_TOOL}')

        local.sync(self._build_file('bin', const.ALDABA_CLI), cli_bin_dir, '-avL')
        local.sync(self._build_file('bin', const.EVMONE_SO), cli_bin_dir, '-avL')
        local.sync(self._build_file('bin', const.DORA_SO), cli_bin_dir, '-avL')
        local.sync(self._build_file('bin', const.ALDABA_VERSION), cli_bin_dir)
    ...
    def initialize_conf(self, conn: Context):
        ...
        # set aldaba conf in etcd
        logs.info('set aldaba conf in etcd')
        ...
        for key, file in json_files.items():
            logs.info(f'set {key}')
            # TODO 使用aldaba_cli
            #  conn.run(f'cd {cli_bin_dir}; ./aldaba_cli meta -c cli.conf -t cp {file} etcd:/{key}')
            conn.run(
    ...
    def deploy(self, service=None):
        ...
        if deploy_client_host is not None:
            # deploy aldaba client at local host and controller(light) host
    ...
    def clone_from(self, is_cold) ->  Dict[str, List[str]]:
        ...
        if is_cold:
            ...
            with Connection(light_instance.ip, user=self.run_user) as conn:
                cli_bin_dir = join(self.remote_client_dir, 'bin')
                ret = conn.run(f'cd {cli_bin_dir}; LD_PRELOAD=./{const.EVMONE_SO} ./aldaba_cli snapshot -d {snapshot_name}')
                if not ret.ok:
  ...
  },
  {
    ...
    def stop_instance(self, instance: Instance, conn: Connection):
        ...
        if self.enable_docker:
            ...
            # TODO use --stop to stop aldaba service
    ...
    def start_instance(self, instance: Instance, conn: Connection):
        ...
        if self.enable_docker:
            ...
            if exists(conn, join(instance.dir, 'conf/launch.conf')):
                if self.is_light:
                    if self.chain_protocol == const.PROTOCOL_EVM or self.chain_protocol == const.PROTOCOL_ALL:
                        cmd = f"cd {work_dir}; LD_PRELOAD=./{const.EVMONE_SO}:./{const.DORA_SO} ./{binary} -c ../conf/launch.conf -d"
                    else:
                        cmd = f"cd {work_dir}; ./{binary} -c ../conf/launch.conf -d"
                else:
                    if self.chain_protocol == const.PROTOCOL_EVM or self.chain_protocol == const.PROTOCOL_ALL:
                        cmd = f"cd {work_dir}; LD_PRELOAD=./{const.EVMONE_SO}:./{const.DORA_SO} ./{binary} -c ../conf/launch.conf -s {instance.service} -d"
                    else:
        ...
        if self.is_light:
            ...
            try:
                with Connection(self.cluster[const.SERVICE_LIGHT].ip, user=self.run_user) as conn:
                    ...
                    # genesis
                    cli_bin_dir = join(self.remote_client_dir, 'bin')
                    cmd = f'cd {cli_bin_dir}; LD_PRELOAD=./{const.EVMONE_SO} ./aldaba_cli genesis -g ../conf/genesis.conf --spec 0 -s {const.MYGRID_GENESIS_CONFIG_FILENAME}'
                    logs.info(f'{conn.host}: {cmd}')
                    conn.run(cmd)
            except Exception as e:
                logs.error('{}'.format(e))
    ...
    def cli_dbquery(self, query_arg: str):
        try:
            ...
            with Connection(light_instance.ip, user=self.run_user) as conn:
                ...
                try:
                    cli_bin_dir = join(self.remote_client_dir, 'bin')
                    command = f'cd {cli_bin_dir}; LD_PRELOAD=./{const.EVMONE_SO} ./aldaba_cli dbquery {query_arg} -o {remote_temp_file}; cat {remote_temp_file}'
    ...
    def cli_dumpstate(self, arg: str):
        try:
            ...
            with Connection(light_instance.ip, user=self.run_user) as conn:
                ...
                try:
                    cli_bin_dir = join(self.remote_client_dir, 'bin')
                    command = f'cd {cli_bin_dir}; LD_PRELOAD=./{const.EVMONE_SO} ./aldaba_cli dumpstate {arg} -o {remote_temp_file}; cat {remote_temp_file}'
                    result = conn.run(command, hide=True)
                    if result.failed:
