...
from aldaba_ops.toolkit.schemas.deploy import *
from aldaba_ops.toolkit.schemas.domain import *
from aldaba_ops.toolkit import const, logs, utils, aldaba
from aldaba_ops.toolkit.conn_group import local

from aldaba_ops.toolkit.utils import to_serializable
...
class Generator(object):
    ...
    def _generate_prikey(self, key_type:str, key_dir: str, key_file: str = "new.key", key_passwd: str = "123abc"):
        ...
        if key_type not in [key_type.value for key_type in aldaba.KeyType]:
            logs.error(f'{key_type} is not supported, please select from {[key_type.value for key_type in aldaba.KeyType]}')
            return


        if key_type == aldaba.KeyType.PRIME256V1.value:
        ...
        elif key_type == aldaba.KeyType.RSA.value:
        ...
        elif key_type == aldaba.KeyType.SM2.value:
            logs.error(f'{key_type} is not supported')

        elif key_type == aldaba.KeyType.BLS12381.value:
            # OpenSSL does not support BLS key generation, so we use `aldaba_cli` here
            # no passwd
            aldaba_cli_path = self._build_file('bin', const.ALDABA_CLI)
            evmone_so_path = self._build_file('bin', const.EVMONE_SO)

            # The output is two line:
            # "PRIVKEY:0x4002xxxxxxxx"
            # "PUBKEY:0x4003xxxxxxxx"
            ret = local.run(f"LD_PRELOAD={evmone_so_path} {aldaba_cli_path} crypto -t gen-key -a bls12381 | tail -n 2") # last 2-lines are prikey and pubkey
    ...
    def _get_pubkey(key_type:str, prikey_path: str, key_passwd: str = "123abc") -> (str, str):
        # pubkey prefix can refer to document: https://yuque.antfin-inc.com/antchain/aldaba-ng/tfcxty
    ...
    def _generate_domain(self, domain_label: str, dinfo: DomainSummary) -> Domain:
        ...
        if self._deploy.enable_dora:
            domain.enable_dora = True
        ...
        for desc in dinfo.cluster:
            ...
            for inst_name in desc.instances.split(','):
                ...
                if instance.service == const.SERVICE_ETCD:
                    ...
                    instance.args = self._deploy.aldaba.args + \
                        ['-s', instance.service, '-d']
                    instance.env = {
                        **self._deploy.aldaba.env,
                    ...
                    if instance.service == const.SERVICE_LIGHT:
                        ...
                        instance.args = self._deploy.aldaba.args + ['-d']
                    ...
                    if instance.service in [const.SERVICE_COMPUTE, const.SERVICE_LIGHT]:
                        # dora
                        instance.env['ENABLE_DORA'] = '1' if domain.enable_dora else '0'
    ...
    def run(self):
        ...
        if self._deploy.use_latest_version:
            aldaba_version_file = self._build_file('bin', const.ALDABA_VERSION)
            aldaba_version_file_bak = self._build_file('bin', f"{const.ALDABA_VERSION}_bak")
                        
            # 备份文件
            if not exists(aldaba_version_file_bak): 
                local.run(f'cp -rf {aldaba_version_file} {aldaba_version_file_bak}')

            with open(aldaba_version_file, "r") as file:
                data = json.load(file)  
            max_key = max(data, key=lambda k: data[k]["version"])
            result = {max_key: data[max_key]}
            result[max_key]["epoch"] = 0
            logs.info(f'dump VERSION file at: {aldaba_version_file}')
            with open(aldaba_version_file, "w") as file:
