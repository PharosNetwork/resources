#!/usr/bin/env python3
# coding=utf-8
from pharos_ops.toolkit import utils
...
PHAROS_OSS_PREFIX = "https://develop.pharosnetwork.xyz"
...
def upload_log(l_fp, r_fp):
    ...
    #     "{1}/{2}"
    # """.format(l_fp, 'ftp://pharosftp.xyz/test/log', r_fp))
    # upload_oss
    print("download oss tool")
    utils.must_exec_run("curl -O " + PHAROS_OSS_PREFIX + "/mychain-0.10/ci_config/ossutil64")
    utils.must_exec_run("curl -O " + PHAROS_OSS_PREFIX + "/mychain-0.10/ci_config/ossconfig")
    oss_target = "oss://antsys-mychain/platform-ci-test/pharos/{}".format(r_fp)
